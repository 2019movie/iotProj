...
| ---------- |------------------------------------------------------------------|--------------------------------------------------------------|
| `CarPark`    | capacity, sensors, displays, car_park_address, car_number_plates | car_enter, car_exit, add_sensor, add_display, update_display |
| `Sensor`     | id, sensor_type, car_number_plate_value                          | detect_car, update_car_park                                  |
| `Display`    | id, display_value                                                | update_display                                               |
...
![Added stubs for classes](images/src3.JPG)
...
![addition evidence](./images/src4.JPG)
...
s2
```
![git tag screenshot](./images/src5.JPG)
### Relate the classes
...
> CarPark class, it must contain the basic attributes of carpark such as number of available bays.
>Q. Which class is responsible for the current temperature (and why)?
> Sensor class, temperature detect by sensor. Sensor class is suitable, providing that sensor has that function.
>Q. Which class is responsible for the time (and why)?
> Carpark class holds the time to act as a single source of truth for all carpark operations to reference time.
--------
...
![Added methods to the car park class](images/src6.JPG)
...
>      `CarPark class, it hold the single source of truth and keep track of available bays`
>    - _The current temperature_
>      `Sensor class, it has the sensing unit provide the temperature reading when requested`
>    - _The time_
>      `CarPark class, it access datetime class, it act as the single source of truth to make request for current time when needed.
>
> 2. **What is the difference between an attribute and a property?**
>    `attribute access the value of a variable, property can access a value derived from variables and protect the value from modification attempts.`
>
> 3. **Why do you think we used a dictionary to hold the data we passed the display? List at least one advantage and one disadvantage of this approach.**
>    `Use dictionary is more effective to call the information needed to display directly by its key. The disadvantage is more complex to develop or troubleshoot when error.`
...
![Unit tests](images/src7.JPG)

![Unit tests after fix](images/src8.JPG)
...
![Unit tests result](images/9.JPG)

![Unit tests after fix](images/10.JPG)
...
> In case of 'TypeError' test as expected value, it should test in CarPark unit test. For auditing purpose, auditor will look for that test under carpark unit test file logically. It is part of communication to someone else pick up the work in progress. 
...
![Added methods to the car park class](images/src11.JPG)
...
1. Add a screenshot of the output of the `main.py` file:
   ...
   ![Main.py output](images/src12.JPG)
...
4. Release your code on GitHub. You can do this by going to the releases section and selecting "Create a new release". Give the release a title ("Project Submission") and description. Then click "Publish release". Include a screenshot of the release:
   ...
   ![Create a release](images/src13.JPG)

   ![Publish a release](images/src14.JPG)
